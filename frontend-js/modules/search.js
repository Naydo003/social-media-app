...
import DOMpurify from 'dompurify'    // used when we insert HTML with template literals to stop and malicious javascript being executed that made it into the database. (Although this should never happen because we sanitise on the way in). Note could have used sanitize-html but that is a large package that needs to be sent over. dompurify is only 40kb.
